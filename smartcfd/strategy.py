...
"""Feature engineering is centralized in smartcfd.features.create_features."""
...
class InferenceStrategy(Strategy):
    ...
    def evaluate(self, symbol: str, regime: str, historical_data: pd.DataFrame) -> Optional[Dict[str, Any]]:
        ...
        if confidence >= self.app_config.trade_confidence_threshold:
            if pred_label == 1 or pred_label == '1':
                action = {'action': 'trade', 'side': 'buy', 'confidence': confidence}
            elif pred_label == 2 or pred_label == '2':
                action = {'action': 'trade', 'side': 'sell', 'confidence': confidence}
