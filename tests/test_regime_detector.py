...
from smartcfd.config import AppConfig, RegimeConfig

@pytest.fixture
def regime_detector():
    """Provides a default RegimeDetector for tests."""
    app = AppConfig(min_data_points=20)
    reg = RegimeConfig(short_window=5, long_window=20)
    return RegimeDetector(app, reg)
...
def test_detect_regime_none_data(regime_detector):
    ...
    regime = regime_detector.detect(None)
    assert regime == MarketRegime.UNDEFINED

def test_init_invalid_windows():
    """Tests that the constructor raises an error for invalid window sizes."""
    with pytest.raises(ValueError):
        RegimeDetector(AppConfig(), RegimeConfig(short_window=20, long_window=20))
    with pytest.raises(ValueError):
        RegimeDetector(AppConfig(), RegimeConfig(short_window=30, long_window=20))
